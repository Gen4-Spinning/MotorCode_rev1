...
void EXTI2_IRQHandler(void)
{
  /* USER CODE BEGIN EXTI2_IRQn 0 */
	if (__HAL_GPIO_EXTI_GET_FLAG(ENC_INDEX_Pin))
		{ //HAL_GPIO_TogglePin(GPIOB,TX_DEBUG_Pin);
		  encoderIndexCounter++;
...
void TIM7_IRQHandler(void)
{
  ...
  //To get one full rotation within 1 ms, we need to be doing > 1000rps -> 60000rpm! since
  //thats too high, we dont need to worry about it. for us at 2500 rpm Max, the Hz = 2500/60 = 41.66 rps
  // so in 1 ms we would have rotated 0.042 rotations, about 85 counts.(MAX)
